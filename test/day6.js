/* global expect, fs, path, rewire, describe, it */
...
describe('Day 6', () => {
  ...
  describe('#redistribute', () => {
    it('should return 2 4 1 2 when array input is 0 2 7 0', (done) => {
      expect(redistribute([0, 2, 7, 0]).toString()).to.be.eq([2, 4, 1, 2].toString());
      done();
    });
    it('should return 3 1 2 3 when array input is 2 4 1 2', (done) => {
      expect(redistribute([2, 4, 1, 2]).toString()).to.be.eq([3, 1, 2, 3].toString());
      done();
    });
  });
});
